...
function documentArgFeatureWorker(context, document, arg, isValidSourceMap, transformArg, worker, transform, combineResult) {
    return __awaiter(this, void 0, void 0, function* () {
            ...
            yield (0, definePlugin_1.visitEmbedded)(vueDocument, embeddeds, (sourceMap) => __awaiter(this, void 0, void 0, function* () {
                ...
                for (const mappedArg of transformArg(arg, sourceMap)) {
                    for (const plugin of plugins) {
                        const embeddedResult = yield worker(plugin, sourceMap.mappedDocument, mappedArg);
                        if (!embeddedResult)
                            continue;
                        const result = yield transform(embeddedResult, sourceMap);
                        if (!result)
                            continue;
                        results.push(result);
                        if (!combineResult)
        ...
        if (results.length === 0 || !!combineResult) {
            ...
            for (const plugin of plugins) {
                const embeddedResult = yield worker(plugin, document, arg);
                if (!embeddedResult)
                    continue;
                const result = yield transform(embeddedResult, undefined);
                if (!result)
                    continue;
                results.push(result);
                if (!combineResult)
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (vueDocument) {
            ...
            yield (0, definePlugin_1.visitEmbedded)(vueDocument, embeddeds, (sourceMap) => __awaiter(this, void 0, void 0, function* () {
                ...
                for (const mappedArg of transformArg(arg, sourceMap)) {
                    for (const plugin of plugins) {
                        const embeddedResult = yield worker(plugin, sourceMap.mappedDocument, mappedArg, sourceMap, vueDocument);
                        if (!embeddedResult)
                            continue;
