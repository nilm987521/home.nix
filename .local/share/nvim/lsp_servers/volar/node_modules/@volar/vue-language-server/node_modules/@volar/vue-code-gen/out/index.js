 ...
 */
function compileSFCTemplate(htmlCode, options = {}, vueVersion) {
    const errors = [];
    const warnings = [];
    let ast;
    try {
        ast = (vueVersion < 3 ? CompilerVue2 : CompilerDOM).compile(htmlCode, Object.assign({ onError: (err) => errors.push(err), onWarn: (err) => warnings.push(err), expressionPlugins: ['typescript'] }, options)).ast;
    }
    catch (e) {
        const err = e;
        errors.push(err);
