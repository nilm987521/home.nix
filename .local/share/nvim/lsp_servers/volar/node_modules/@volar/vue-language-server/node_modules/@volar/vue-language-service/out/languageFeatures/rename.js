...
function register(context) {
    return (uri, position, newName) => {
        return (0, featureWorkers_1.languageFeatureWorker)(context, uri, { position, newName }, function* (arg, sourceMap) {
            for (const [mappedRange, mappedData] of sourceMap.getMappedRanges(arg.position, arg.position, data => typeof data.capabilities.rename === 'object' ? data.capabilities.rename.in : !!data.capabilities.rename)) {
            ...
            let result;
            yield withTeleports(document, arg.position, arg.newName);
            return result;
            function withTeleports(document, position, newName) {
                var _a;
                    ...
                    if (workspaceEdit.changes) {
                        for (const editUri in workspaceEdit.changes) {
                            ...
                            for (const textEdit of textEdits) {
                                ...
                                if (teleport) {
                                    for (const [teleRange, sideData] of teleport.findTeleports(textEdit.range.start, textEdit.range.end, sideData => !!sideData.capabilities.references)) {
                                        ...
                                        const newName_2 = sideData.transformNewName
                                            ...
                                            : newName;
                                        yield withTeleports(teleport.document, teleRange.start, newName_2);
                                    }
                                }
                                if (!foundTeleport) {
                                    if (!result.changes)
            ...
            return embeddedEditToSourceEdit(data, context.vueDocuments);
        }, (workspaceEdits) => {
...
function embeddedEditToSourceEdit(tsResult, vueDocuments) {
    ...
    for (const tsUri in tsResult.changeAnnotations) {
        ...
        const uri = (_b = (_a = vueDocuments.sourceMapFromEmbeddedDocumentUri(tsUri)) === null || _a === void 0 ? void 0 : _a.sourceDocument.uri) !== null && _b !== void 0 ? _b : tsUri;
        vueResult.changeAnnotations[uri] = tsAnno;
    }
    for (const tsUri in tsResult.changes) {
        const tsEdits = tsResult.changes[tsUri];
        for (const tsEdit of tsEdits) {
            for (const vueLoc of vueDocuments.fromEmbeddedLocation(tsUri, tsEdit.range.start, tsEdit.range.end, data => typeof data.capabilities.rename === 'object' ? data.capabilities.rename.out : !!data.capabilities.rename)) {
            ...
            if (vscode.TextDocumentEdit.is(tsDocEdit)) {
                ...
                if (sourceMap) {
                    ...
                    for (const tsEdit of tsDocEdit.edits) {
                        for (const [vueRange, data] of sourceMap.getSourceRanges(tsEdit.range.start, tsEdit.range.end, data => typeof data.capabilities.rename === 'object' ? data.capabilities.rename.out : !!data.capabilities.rename)) {
                ...
                const oldUri = (_d = (_c = vueDocuments.sourceMapFromEmbeddedDocumentUri(tsDocEdit.oldUri)) === null || _c === void 0 ? void 0 : _c.sourceDocument.uri) !== null && _d !== void 0 ? _d : tsDocEdit.oldUri;
                ...
                const uri = (_f = (_e = vueDocuments.sourceMapFromEmbeddedDocumentUri(tsDocEdit.uri)) === null || _e === void 0 ? void 0 : _e.sourceDocument.uri) !== null && _f !== void 0 ? _f : tsDocEdit.uri;
