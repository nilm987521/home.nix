...
exports.getPatchForSlotNode = exports.generate = exports.isIntrinsicElement = exports.transformContext = void 0;
...
function isIntrinsicElement(runtimeMode = 'runtime-dom', tag) {
...
function generate(ts, sourceLang, templateAst, vueVersion, experimentalRuntimeMode, allowTypeNarrowingInEventExpressions, cssScopedClasses = [], htmlToTemplate, searchTexts) {
            ...
            if (isIntrinsicElement(experimentalRuntimeMode, tagName)) {
                for (const tagRange of tagRanges) {
                    ...
                    writeObjectProperty2(tagName, [tagRange], {
                        vueTag: 'template',
                        capabilities: Object.assign(Object.assign({}, capabilitiesSet.tagHover), capabilitiesSet.tagReference),
                    });
                    tsCodeGen.addText(` />);\n`);
                }
            }
        ...
        if (node.type === 1 /* CompilerDOM.NodeTypes.ELEMENT */) {
            ...
            for (const prop of node.props) {
                if (prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */
                    ...
                    && prop.arg.isStatic) {
                    let propName = prop.arg.constType === 3 /* CompilerDOM.ConstantTypes.CAN_STRINGIFY */
                        ? prop.arg.content
                        : prop.arg.loc.source;
            ...
            for (const prop of node.props) {
                ...
                function appendExpressionNode(prop, jsChildNode) {
                    ...
                    if (((_a = prop.exp) === null || _a === void 0 ? void 0 : _a.type) === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */) {
                        if (jsChildNode.type === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */) {
                            writeInterpolation(prop.exp.content, prop.exp.loc.start.offset, {
                                vueTag: 'template',
                                capabilities: capabilitiesSet.all,
                            }, '(', ')');
                        ...
                        else if (jsChildNode.type === 8 /* CompilerDOM.NodeTypes.COMPOUND_EXPRESSION */) {
                            const _exp = prop.exp;
                            const expIndex = jsChildNode.children.findIndex(child => typeof child === 'object' && child.type === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */ && child.content === _exp.content);
                            const expNode = jsChildNode.children[expIndex];
                            let prefix = jsChildNode.children.filter((child, i) => typeof child === 'string' && i < expIndex).map(child => child).join('');
                            let suffix = jsChildNode.children.filter((child, i) => typeof child === 'string' && i > expIndex).map(child => child).join('');
    ...
    function writeProps(node, forRemainStyleOrClass, mode) {
        ...
        for (const prop of node.props) {
            if (prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */
                ...
                if (((_c = prop.arg) === null || _c === void 0 ? void 0 : _c.type) === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */) {
                ...
                && prop.name === 'bind'
                && !prop.arg
                && ((_e = prop.exp) === null || _e === void 0 ? void 0 : _e.type) === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */) {
    ...
    function writeInterpolation(mapCode, sourceOffset, data, prefix, suffix) {
        (0, transform_1.walkInterpolationFragment)(ts, prefix + mapCode + suffix, (frag, fragOffset, isJustForErrorMapping) => {
